...
public class T02_LambdaTest {
	public static void main(String[] args) {
		// 람다식을 사용하지 않았을 경우
		LambdaTestInerface1 lam1 = new LambdaTestInerface1() {
		...
		LambdaTestInerface1 lam2 = () -> System.out.println("람다식 구현 방식입니다.");
		lam2.test(); // 메서드 호출
 ...
 */
		...
		LambdaTestInerface2 lam5 = 
			z -> System.out.println("result = " + (z+500));
		lam5.test(90);
		
		System.out.println("------------------------------------");
		
		LambdaTestInerface3 lam6 = 
			(int x, int y) -> {
				int r = x + y;
				return r;
			};
		...
		LambdaTestInerface3 lam8 = (x, y) -> x + y;
		k = lam8.test(100, 200);
		System.out.println("k = " + k);
		
		LambdaTestInerface3 lam9 = (x, y) -> { return x > y ? x : y; };
		k = lam9.test(100, 200);
		System.out.println("k = " + k);
	}
}
